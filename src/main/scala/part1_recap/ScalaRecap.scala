...
object ScalaRecap extends App {
  ...
  // method notations - infix specially
  1 + 2 // syntactic sugar
  1.+(2)
  ...
  setTimeout(() => println("timeout")) // other org list injected by the compiler

  // conversions
  //1) implicit methods
  case class Person(name: String) {
    def greet: String = s"Hy, my name is $name"
  }
  ...
  List(Person("Bob"), Person("Alice"), Person("Amanda"))
  // => List(Person("Alice"), Person("Amanda"), Person("Bob"))
}
